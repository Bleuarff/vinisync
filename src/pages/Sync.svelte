<script>
  ...
  let syncEnabled = false // whether sync is activate on this device
  let email = ''
  let userkey = ''
...
{:else}
  <div>
    ...
    <input type="email" bind:value={email} placeholder="barney@poivrot.com">

    {#if isLink}
      <label for="">Clef</label>
      <input type="text" bind:value={userkey}>
    {/if}

    <div class="submit-ctnr">
      <button on:submit={setSync}>Synchroniser</button>
    </div>
  </div>
{/if}
